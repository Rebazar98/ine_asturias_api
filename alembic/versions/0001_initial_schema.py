...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_ingestion_raw_lookup",
        "ingestion_raw",
        ["source_type", "source_key", "fetched_at"],
        unique=False,
    )
    ...
    op.create_index(
        "ix_ine_series_normalized_inserted_at",
        "ine_series_normalized",
        ["inserted_at"],
        unique=False,
    )
    op.create_index(
        ...
        "ine_series_normalized",
        ["operation_code", "table_id", "geography_code", "period"],
        unique=False,
    )
    op.create_index(
        "ix_ine_series_normalized_period", "ine_series_normalized", ["period"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "validation_status", sa.String(length=32), server_default="unknown", nullable=False
        ),
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "operation_code", "table_id", name="uq_ine_tables_catalog_operation_table"
        ),
    )
    op.create_index(
        "ix_ine_tables_catalog_operation_code",
        "ine_tables_catalog",
        ["operation_code"],
        unique=False,
    )
    op.create_index(
        "ix_ine_tables_catalog_table_id", "ine_tables_catalog", ["table_id"], unique=False
    )
    op.create_index(
        "ix_ine_tables_catalog_operation_checked",
        "ine_tables_catalog",
        ["operation_code", "last_checked_at"],
        unique=False,
    )
    op.create_index(
        "ix_ine_tables_catalog_operation_status",
        "ine_tables_catalog",
        ["operation_code", "validation_status", "has_asturias_data"],
        unique=False,
    )
